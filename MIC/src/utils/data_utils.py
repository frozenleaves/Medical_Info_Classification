...
def create_data_index_from_directory(data_dir: str, 
                                    output_dir: str,
                                    text_subdir: str = 'texts',
                                    photo_subdir: str = 'photos', 
                                    pathology_subdir: str = 'pathology',
                                    label_file: Optional[str] = None) -> Dict[str, List[Dict]]:
    """
    从目录结构创建数据索引文件
...
def create_data_splits(samples: List[Dict[str, Any]], 
...
def analyze_dataset(data_dir: str, index_files: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    分析数据集统计信息
    ...
    for index_file in index_files:
        ...
        for sample in samples:
                ...
                try:
                    ...
                    if text_file.exists():
                        with open(text_file, 'r', encoding='utf-8') as tf:
                            ...
                            analysis['modality_statistics']['text_lengths'].append(text_length)
                except:
                    pass
            ...
            if sample.get('photo_paths'):
                ...
                analysis['modality_statistics']['photos_per_sample'].append(len(sample['photo_paths']))
            
            if sample.get('pathology_paths'):
                split_analysis['modality_counts']['pathology'] += 1
                analysis['modality_availability']['pathology'] += 1
                analysis['modality_statistics']['pathology_per_sample'].append(len(sample['pathology_paths']))
...
def print_dataset_analysis(analysis: Dict[str, Any]):
    ...
    if stats['photos_per_sample']:
        ...
        print(f"  照片数量/样本: 均值={photos_stats.mean():.1f}, "
...
def visualize_dataset_distribution(analysis: Dict[str, Any], save_path: Optional[str] = None):
    """可视化数据集分布"""
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    
    # 1. 分割分布饼图
    ax1 = axes[0, 0]
    split_sizes = [split_info['num_samples'] for split_info in analysis['splits'].values()]
    ...
    if analysis['modality_statistics']['photos_per_sample']:
        ax4.hist(analysis['modality_statistics']['photos_per_sample'], 
                bins=20, alpha=0.7, edgecolor='black')
        ax4.set_title('每个样本的照片数量分布')
        ax4.set_xlabel('照片数量')
        ax4.set_ylabel('样本数')
    else:
        ax4.text(0.5, 0.5, '无照片数据', ha='center', va='center', transform=ax4.transAxes)
        ax4.set_title('每个样本的照片数量分布')
...
def prepare_data_splits(data_dir: str, 
...
if __name__ == "__main__":
    ...
    for i in range(10):
        # 文本文件
        with open(test_data_dir / "texts" / f"patient_{i:03d}.txt", 'w', encoding='utf-8') as f:
    ...
    try:
        output_dir = prepare_data_splits(
            str(test_data_dir),
            label_file=str(test_data_dir / "labels.json")
        )
