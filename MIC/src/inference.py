...
class MultiModalInferencer:
    ...
    def __init__(
        self, model_path: str, device: str = "auto", config_path: Optional[str] = None
    ):
        """
        初始化推理器
        ...
        if "class_names" in checkpoint and checkpoint["class_names"]:
            ...
            if len(loaded_class_names) != model_num_classes:
                print(
                    f"⚠️  警告: checkpoint中的类别数({len(loaded_class_names)}) "
            ...
            if len(label_map) != model_num_classes:
                print(
                    f"⚠️  注意: 数据集有{len(label_map)}个类别，模型定义{model_num_classes}个类别"
                )
        else:
            self.class_names = self.model_config.get(
                "class_names", [f"Class_{i}" for i in range(model_num_classes)]
            )
        ...
        if self.num_classes != model_num_classes:
            print(
                f"⚠️  严重警告: class_names长度({self.num_classes}) "
    ...
    def _load_model(self) -> tuple:
        ...
        if "model_config" in checkpoint:
            ...
            raise ValueError(
                "无法找到模型配置，请提供config_path或确保模型文件包含配置"
            )
    ...
    def predict_single(
        ...
        result = {
            ...
            "class_probabilities": {
                self.class_names[i]: prob.item() for i, prob in enumerate(predictions)
            },
            "modal_availability": outputs["modal_availability"][0]
            .cpu()
            .numpy()
            .tolist(),
        }
        ...
        if return_attention and "attention_weights" in outputs:
            result["attention_weights"] = self._process_attention_weights(
    ...
    def predict_batch(
        self, samples: List[Dict[str, Any]], batch_size: int = 8
    ) -> List[Dict[str, Any]]:
        """
        批量预测
    ...
    def _prepare_single_sample(
        self, text: str, image_paths: List[str], pathology_paths: List[str]
    ) -> Dict[str, Any]:
        """准备单个样本的输入数据"""
        # 文本处理
    ...
    def _build_batch_dict(
        ...
        pathology_list: List[List[torch.Tensor]],
    ) -> Dict[str, Any]:
        """构建批次数据字典"""
        ...
        # 病理数据
        max_patches = (
            max([len(patches) for patches in pathology_list]) if pathology_list else 0
        )
        ...
        return {"text": text_data, "photos": photo_data, "pathology": pathology_data}
    ...
    def _split_batch_outputs(
        ...
        for i in range(batch_size):
            sample_output = {
                ...
                "modal_availability": batch_outputs.get("modal_availability", [None])[
                    i
                ],
            }
    ...
    def _process_attention_weights(
        ...
        for modal_name, weights in attention_weights.items():
            if isinstance(weights, torch.Tensor):
                processed[modal_name] = (
                    weights[0].cpu().numpy().tolist()
                )  # 取第一个样本
    ...
    def generate_report(
        self, result: Dict[str, Any], sample_info: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        生成预测报告
        ...
        for i, (modal_name, available) in enumerate(
            zip(modal_names, modal_availability)
        ):
...
def main():
    ...
    parser.add_argument("--text", type=str, default="", help="病历文本")
    parser.add_argument("--images", type=str, nargs="*", help="口腔照片路径列表")
    parser.add_argument("--pathology", type=str, nargs="*", help="病理切片路径列表")
    parser.add_argument(
        "--output_dir", type=str, default="./inference_results", help="输出目录"
    )
    ...
    inferencer = MultiModalInferencer(
        model_path=args.model_path, device=args.device, config_path=args.config_path
    )
