...
class AttentionPooling(nn.Module):
    ...
    def forward(
        ...
        # 计算注意力权重
        attention_weights = self.attention_net(
            features
        )  # [batch_size, num_patches, num_heads]
        ...
        for head in range(self.num_heads):
            head_weights = attention_weights[:, :, head].unsqueeze(
                -1
            )  # [batch_size, num_patches, 1]
            head_pooled = torch.sum(
                gated_features * head_weights, dim=1
...
class MILClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.attention_pooling = AttentionPooling(
            input_dim=input_dim, hidden_dim=hidden_dim // 2
        )
    ...
    def forward(
        ...
        """
        # 实例级预测
        instance_logits = self.instance_classifier(
            features
        )  # [batch_size, num_patches, num_classes]
...
class PathologyEncoder(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        self.feature_projection = nn.Linear(
            self.patch_feature_dim, self.mil_feature_dim
        )
    ...
    def _load_patch_extractor(self) -> nn.Module:
        ...
        if isinstance(self.patch_backbone, str) and (
            self.patch_backbone.startswith("/")
            or self.patch_backbone.startswith("./")
            or Path(self.patch_backbone).exists()
        ):
            # 本地路径处理
    ...
    def _load_local_patch_extractor(self) -> nn.Module:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"成功使用transformers加载本地patch提取器, 特征维度: {model.num_features}"
                )
                return model
    ...
    def forward(
        self, patches: torch.Tensor, masks: torch.Tensor, return_attention: bool = False
    ) -> Dict[str, torch.Tensor]:
        """
        前向传播
        ...
        patches_flat = patches.view(
            batch_size * max_patches, channels, patch_h, patch_w
        )
        ...
        patch_features = patch_features.view(
            batch_size, max_patches, self.patch_feature_dim
        )
    ...
    def get_attention_heatmap(
        self, patches: torch.Tensor, masks: torch.Tensor, coordinates: List[List[Tuple]]
    ) -> List[np.ndarray]:
        """
        生成注意力热力图
        ...
        with torch.no_grad():
            ...
            for i, coords in enumerate(coordinates):
                ...
                sample_attention = (
                    attention_weights[i, : len(coords), 0].cpu().numpy()
                )  # 取第一个head
    ...
    def forward(
        ...
        if mask is not None:
            ...
            return masked_features.sum(dim=1) / (
                mask.sum(dim=1, keepdim=True).float() + 1e-8
            )
    ...
    def __init__(
        ...
        if self.use_pos_encoding:
            self.pos_encoding = nn.Parameter(
                torch.randn(1, 1000 + 1, hidden_dim)
            )  # +1 for CLS
...
if __name__ == "__main__":
    ...
    # 测试注意力可视化
    dummy_coords = [
        [(i * 10, j * 10) for i in range(15) for j in range(1)],
