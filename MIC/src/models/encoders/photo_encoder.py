...
class MultiImageAttention(nn.Module):
    ...
    def forward(self, 
        ...
        # 重塑为多头形式
        Q = Q.view(batch_size, num_images, self.num_heads, self.head_dim).transpose(1, 2)
        K = K.view(batch_size, num_images, self.num_heads, self.head_dim).transpose(1, 2)
        V = V.view(batch_size, num_images, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        if mask is not None:
            ...
            expanded_mask = mask.unsqueeze(1).unsqueeze(2)  # [batch_size, 1, 1, num_images]
        ...
        if mask is not None:
            # 计算每张图片的重要性权重
            global_weights = attention_weights.mean(dim=1).mean(dim=1)  # [batch_size, num_images]
            global_weights = global_weights.masked_fill(~mask, 0)
            global_weights = global_weights / (global_weights.sum(dim=1, keepdim=True) + 1e-8)
        else:
            global_weights = torch.ones(batch_size, num_images, device=image_features.device) / num_images
...
class PhotoEncoder(nn.Module):
    ...
    def _load_backbone(self) -> nn.Module:
        ...
        if isinstance(self.backbone_name, str) and (
            self.backbone_name.startswith('/') or 
            self.backbone_name.startswith('./') or 
            Path(self.backbone_name).exists()
        ):
            # 本地路径处理
    ...
    def _load_local_backbone(self) -> nn.Module:
        ...
        try:
            ...
            try:
                ...
                print(f"成功使用transformers加载本地模型, 特征维度: {model.num_features}")
                return model
                ...
                try:
                    ...
                    if config_path.exists():
                        ...
                        if 'vit' in model_type.lower():
                            ...
                            print(f"成功从本地加载ViT模型, 特征维度: {model.num_features}")
                            return model
                ...
                if isinstance(images, list):
                    ...
                    if max_len == 0:
                        ...
                        if images_tensor.shape[0] < self.max_images:
                            ...
                            padding = torch.zeros(
                                padding_size, *images_tensor.shape[1:],
                                device=images_tensor.device
                            )
                            images_tensor = torch.cat([images_tensor, padding], dim=0)
                        
                        # 添加batch维度
                        images_tensor = images_tensor.unsqueeze(0)  # [1, max_images, 3, H, W]
                        
                        # 创建掩码
                        mask = torch.zeros(1, self.max_images, dtype=torch.bool, device=images_tensor.device)
                        mask[0, :max_len] = True
...
class MedicalPhotoEncoder(PhotoEncoder):
    ...
    def forward(self, images: torch.Tensor, masks: torch.Tensor, 
        ...
        if self.medical_preprocessing and hasattr(self, 'medical_enhancement'):
            ...
            images = enhanced_images.view(batch_size, max_images, channels, height, width)
