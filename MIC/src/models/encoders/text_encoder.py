...
class TextEncoder(nn.Module):
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                ...
                local_files_only=Path(self.model_name).exists() if isinstance(self.model_name, str) else False
            )
    ...
    def _has_cls_token(self) -> bool:
        """检查模型是否有[CLS] token"""
        return hasattr(self.tokenizer, 'cls_token') and self.tokenizer.cls_token is not None
    ...
    def get_text_attention_weights(self, texts: List[str]) -> Tuple[torch.Tensor, List[List[str]]]:
        """获取文本注意力权重"""
        self.eval()
        with torch.no_grad():
...
class MedicalTextEncoder(TextEncoder):
    ...
    def forward(self, texts: List[str], return_attention: bool = False) -> Dict[str, torch.Tensor]:
        """医学文本前向传播"""
