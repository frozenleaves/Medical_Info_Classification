...
class MultiModalMedicalClassifier(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        self.pathology_encoder = create_pathology_encoder(
            config.get("pathology_encoder", {})
        )
    ...
    def forward(
        ...
        # 处理缺失模态
        text_features = self._handle_missing_modal(
            text_features, modal_availability[:, 0]
        )
        photo_features = self._handle_missing_modal(
            photo_features, modal_availability[:, 1]
        )
        pathology_features = self._handle_missing_modal(
            pathology_features, modal_availability[:, 2]
        )
        ...
        if self.use_auxiliary_loss:
            ...
            if modal_availability[:, 0].any():  # 有文本模态
                aux_results["text_logits"] = self.auxiliary_classifiers["text"](
                    text_features
                )

            if modal_availability[:, 1].any():  # 有照片模态
                aux_results["photo_logits"] = self.auxiliary_classifiers["photo"](
                    photo_features
                )

            if modal_availability[:, 2].any():  # 有病理模态
                aux_results["pathology_logits"] = self.auxiliary_classifiers[
                    "pathology"
                ](pathology_features)
        ...
        for i in range(batch_size):
            if not (modal_availability[i] & dropout_mask[i]).any():
                ...
                if len(available_modals) > 0:
                    selected_modal = available_modals[
                        torch.randint(0, len(available_modals), (1,))
                    ]
        ...
        try:
            ...
            return {
                "features": torch.zeros(
                    batch_size, self.text_encoder.embedding_dim, device=device
                )
            }

    def _encode_photos(
        self, photo_data: Dict[str, Any], return_attention: bool = False
    ) -> Dict[str, torch.Tensor]:
        """编码照片数据"""
        try:
            return self.photo_encoder(
                photo_data["images"], photo_data["masks"], return_attention
            )
        except Exception as e:
            ...
            return {
                "features": torch.zeros(
                    batch_size, self.photo_encoder.feature_dim, device=device
                )
            }

    def _encode_pathology(
        self, pathology_data: Dict[str, Any], return_attention: bool = False
    ) -> Dict[str, torch.Tensor]:
        """编码病理数据"""
        try:
            return self.pathology_encoder(
                pathology_data["patches"], pathology_data["masks"], return_attention
            )
        except Exception as e:
            ...
            return {
                "features": torch.zeros(
                    batch_size, self.pathology_encoder.mil_feature_dim, device=device
                )
            }

    def _handle_missing_modal(
        self, features: torch.Tensor, availability: torch.Tensor
    ) -> torch.Tensor:
        """处理缺失模态"""
        # 将不可用模态的特征置零
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "classifier": {"type": "mlp", "hidden_dims": [256, 128], "dropout": 0.3},
    }
    ...
    test_batch = {
        ...
        "pathology": {
            ...
            "coordinates": [
                [(i, j) for i in range(15) for j in range(1)],
