...
class MultiModalTransformer(nn.Module):
    ...
    def forward(
        ...
        modal_mask: Optional[torch.Tensor] = None,
        ...
        multimodal_features = torch.stack(
            [text_proj, photo_proj, pathology_proj], dim=1
        )
        ...
        fused_features = self.transformer(
            multimodal_features, src_key_padding_mask=attention_mask
        )  # [batch_size, 3, feature_dim]
        ...
        if modal_mask is not None:
            ...
            modal_weights = modal_weights / (
                modal_weights.sum(dim=1, keepdim=True) + 1e-8
            )
...
class BilinearFusion(nn.Module):
    ...
    def __init__(self, dim1: int, dim2: int, output_dim: int, dropout: float = 0.1):
        super().__init__()
...
class MultiModalFusionModule(nn.Module):
    ...
    def _create_bilinear_fusion(self) -> nn.Module:
        ...
        class BilinearFusionWrapper(nn.Module):
            def __init__(self, fusion_dim):
                super().__init__()
                # 两两双线性融合
                self.text_photo_fusion = BilinearFusion(
                    fusion_dim, fusion_dim, fusion_dim
                )
                self.text_pathology_fusion = BilinearFusion(
                    fusion_dim, fusion_dim, fusion_dim
                )
                self.photo_pathology_fusion = BilinearFusion(
                    fusion_dim, fusion_dim, fusion_dim
                )
            ...
            def forward(
                self, text_features, photo_features, pathology_features, modal_mask=None
            ):
                # 两两融合
                text_photo = self.text_photo_fusion(text_features, photo_features)
                text_pathology = self.text_pathology_fusion(
                    text_features, pathology_features
                )
                photo_pathology = self.photo_pathology_fusion(
                    photo_features, pathology_features
                )

                # 拼接融合
                combined = torch.cat(
                    [text_photo, text_pathology, photo_pathology], dim=1
                )
    ...
    def forward(
        ...
        photo_features: torch.Tensor,
        pathology_features: torch.Tensor,
        modal_availability: Optional[torch.Tensor] = None,
    ) -> Dict[str, torch.Tensor]:
        """
        前向传播
        ...
        if self.fusion_strategy == "transformer":
            ...
            concatenated = torch.cat(
                [aligned_text, aligned_photo, aligned_pathology], dim=1
            )
...
class AdaptiveModalWeighting(nn.Module):
    ...
    def forward(
        self, modal_features: torch.Tensor, modal_mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        前向传播
        ...
        if modal_mask is not None:
            ...
            global_features = masked_features.sum(dim=1) / (
                modal_mask.sum(dim=1, keepdim=True) + 1e-8
            )
        else:
            global_features = modal_features.mean(dim=1)

        # 预测权重
        modal_weights = self.weight_predictor(
            global_features
...
if __name__ == "__main__":
    ...
    result = fusion_module(
        text_features, photo_features, pathology_features, modal_mask
    )
