...
class MetricCalculator:
    ...
    def compute(self) -> Dict[str, float]:
        ...
        if self.probabilities:
            ...
            metrics.update(self._compute_probability_metrics(true_labels, probabilities))
    ...
    def _compute_basic_metrics(self, predictions: np.ndarray, true_labels: np.ndarray) -> Dict[str, float]:
        """计算基本分类指标"""
        metrics = {}
    ...
    def _compute_confusion_matrix_metrics(self, predictions: np.ndarray, true_labels: np.ndarray) -> Dict[str, float]:
        """计算基于混淆矩阵的指标"""
        metrics = {}
    ...
    def _compute_probability_metrics(self, true_labels: np.ndarray, probabilities: np.ndarray) -> Dict[str, float]:
        """计算基于概率的指标"""
        metrics = {}
        ...
        try:
            ...
            for k in [2, 3]:
                if k < self.num_classes:
                    top_k_acc = self._compute_top_k_accuracy(true_labels, probabilities, k)
    ...
    def _compute_balance_metrics(self, predictions: np.ndarray, true_labels: np.ndarray) -> Dict[str, float]:
        """计算类别平衡相关指标"""
        metrics = {}
        ...
        # 类别分布差异
        true_dist = np.bincount(true_labels, minlength=self.num_classes) / len(true_labels)
        pred_dist = np.bincount(predictions, minlength=self.num_classes) / len(predictions)
    ...
    def _compute_top_k_accuracy(self, true_labels: np.ndarray, probabilities: np.ndarray, k: int) -> float:
        """计算Top-K准确率"""
    ...
    def _compute_cohen_kappa(self, predictions: np.ndarray, true_labels: np.ndarray) -> float:
        """计算Cohen's Kappa系数"""
        cm = confusion_matrix(true_labels, predictions)
        n = np.sum(cm)
...
class MedicalMetrics:
    ...
    @staticmethod
    def compute_diagnostic_metrics(tp: int, tn: int, fp: int, fn: int) -> Dict[str, float]:
        """
        计算诊断相关指标
    ...
    @staticmethod
    def compute_roc_metrics(true_labels: np.ndarray, 
...
class MultiModalMetrics:
    ...
    def update(self, 
        ...
        if 'auxiliary' in outputs:
            ...
            for modal_name in ['text', 'photo', 'pathology']:
                ...
                if logits_key in aux_outputs:
                    aux_probs = torch.softmax(torch.tensor(aux_outputs[logits_key]), dim=1).numpy()
        ...
        if modal_availability is not None:
            ...
            self.modal_availability_stats['text_available'] += modal_availability[:, 0].sum()
            self.modal_availability_stats['photo_available'] += modal_availability[:, 1].sum()
            self.modal_availability_stats['pathology_available'] += modal_availability[:, 2].sum()
    ...
    def compute(self) -> Dict[str, Any]:
        ...
        if total > 0:
            results['modal_availability'] = {
                'text_coverage': self.modal_availability_stats['text_available'] / total,
                'photo_coverage': self.modal_availability_stats['photo_available'] / total,
                'pathology_coverage': self.modal_availability_stats['pathology_available'] / total,
                'total_samples': total
            }
