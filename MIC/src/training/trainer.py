...
class MultiModalTrainer:
    ...
    def __init__(self, 
        ...
        # 评估指标
        class_names = model_config.get('class_names', [f'Class_{i}' for i in range(model_config.get('num_classes', 6))])
        self.metrics = MultiModalMetrics(model_config.get('num_classes', 6), class_names)
        ...
        if hasattr(train_dataloader.dataset, 'label_map'):
            ...
            for i in range(model_num_classes):
                if i in self.idx_to_label:
                    ...
                    self.logger.warning(f"类别索引{i}在数据中不存在，使用占位符: {placeholder}")
            
            self.logger.info(f"从数据集获取标签映射: {self.label_map}")
            self.logger.info(f"类别名称列表({len(self.class_names)}): {self.class_names}")
            
        else:
            self.logger.warning("数据集没有label_map属性，使用默认类别名称")
            self.label_map = None
            self.class_names = [f'Class_{i}' for i in range(self.model_config.get('num_classes', 6))]
        ...
        for batch_idx, batch in enumerate(progress_bar):
            try:
                ...
                with autocast(enabled=self.use_amp):
                    ...
                    loss_dict = self.criterion(outputs, targets, batch.get('modal_availability'))
                ...
                modal_availability_np = batch.get('modal_availability', 
    ...
    def _save_checkpoint(self, val_metrics: Dict[str, float]) -> bool:
        ...
        checkpoint = {
            ...
            # 保存标签映射信息
            'label_map': getattr(self, 'label_map', None),  # {'class_A': 0, 'class_B': 1, ...}
            'class_names': getattr(self, 'class_names', None),  # ['class_A', 'class_B', ...]
            'idx_to_label': getattr(self, 'idx_to_label', None)  # {0: 'class_A', 1: 'class_B', ...}
        }
        ...
        if is_best:
            ...
            self.logger.info(f"New best model saved with {self.monitor_metric}: {current_metric:.4f}")
        
        # 定期保存
        if self.save_config.get('save_every_n_epochs', 0) > 0:
            if self.current_epoch % self.save_config['save_every_n_epochs'] == 0:
                torch.save(checkpoint, self.save_dir / f'epoch_{self.current_epoch}.pth')
    ...
    def _load_checkpoint(self, checkpoint_path: str):
        ...
        self.logger.info(f"Checkpoint loaded from {checkpoint_path}, resuming from epoch {self.current_epoch}")
