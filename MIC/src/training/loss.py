...
class MultiModalLoss(nn.Module):
    ...
    def __init__(self, loss_config: Dict[str, Any]):
        ...
        self.loss_weights = loss_config.get(
            "loss_weights",
            {"main_loss": 1.0, "text_aux": 0.1, "photo_aux": 0.1, "pathology_aux": 0.1},
        )
        ...
        if "auxiliary" in outputs:
            ...
            if "pathology_logits" in aux_outputs:
                if modal_availability is None or modal_availability[:, 2].any():
                    pathology_aux_loss = self.aux_loss(
                        aux_outputs["pathology_logits"], targets
                    )
                    losses["pathology_aux_loss"] = pathology_aux_loss
                    total_loss += (
                        self.loss_weights["pathology_aux"] * pathology_aux_loss
                    )
    ...
    def _compute_contrastive_loss(
        self, features: Dict[str, torch.Tensor], targets: torch.Tensor
    ) -> torch.Tensor:
        """计算对比学习损失"""
        # 使用融合特征进行对比学习
...
class WeightedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if (
            ...
            and self.class_weights.device != inputs.device
        ):
            self.class_weights = self.class_weights.to(inputs.device)

        return F.cross_entropy(
            inputs, targets, weight=self.class_weights, reduction=self.reduction
        )
...
def create_loss_function(config: Dict[str, Any]) -> nn.Module:
    ...
    if loss_type == "multimodal":
        ...
        return FocalLoss(
            alpha=config.get("alpha", None), gamma=config.get("gamma", 2.0)
        )
    elif loss_type == "label_smoothing":
        return LabelSmoothingCrossEntropy(
            num_classes=config.get("num_classes", 6),
        ...
        return WeightedCrossEntropyLoss(class_weights=config.get("class_weights", None))
...
if __name__ == "__main__":
    ...
    model_outputs = {
        ...
        "features": {"fused": torch.randn(batch_size, 512)},
    }
