...
TEST_CONFIG = {
    ...
    'sample_data': {
        ...
        'expected_files': {
            ...
            'photos': ['photos/patient_001/photo1.png', 'photos/patient_001/photo2.png'],
            'pathology': ['pathology/patient_001/slide1.tif', 'pathology/patient_001/slide2.tif']
        }
    }
}
...
class DataLoadingTester:
    ...
    def test_data_transforms(self):
        ...
        try:
            from MIC.src.data.transforms import get_transforms, TextTransforms, PhotoTransforms, PathologyTransforms
            ...
            # 2. 测试文本变换
            text_transform = TextTransforms({'max_length': 256, 'enable_augment': False})
    ...
    def test_data_utils(self):
        ...
        try:
            ...
            # 2. 测试数据分割
            splits = create_data_splits(samples, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2)
            print(f"✅ 数据分割测试成功: train={len(splits['train'])}, val={len(splits['val'])}, test={len(splits['test'])}")
    ...
    def test_dataloader(self):
        ...
        try:
            ...
            # 2. 创建数据加载器
            dataloaders = create_dataloaders(str(TEST_CONFIG['data_dir']), config, use_weighted_sampling=False)
        ...
        try:
            ...
            if labels_file.exists():
                ...
                if actual_label == expected_label:
                    print(f"✅ 标签验证成功: {sample_data['patient_id']} -> {actual_label}")
                else:
    ...
    def test_end_to_end_pipeline(self):
        ...
        try:
            ...
            if sample['photos']:
                print(f"    照片tensor形状: {sample['photos'][0].shape if hasattr(sample['photos'][0], 'shape') else 'N/A'}")
            print(f"  病理处理: {len(sample['pathology']['patches'])} 个patches")
            if sample['pathology']['patches']:
                patch_shape = sample['pathology']['patches'][0].shape if hasattr(sample['pathology']['patches'][0], 'shape') else 'N/A'
        ...
        train_samples = [
            {
                ...
                "pathology_paths": [
                    {
                        "path": "pathology/patient_001/slide1.tif",
                        "roi": None
                    },
                    {
                        "path": "pathology/patient_001/slide2.tif", 
                        "roi": None
                    }
                ]
            }
