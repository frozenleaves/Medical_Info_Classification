...
def test_single_sample_overfitting():
    ...
    try:
        ...
        from MIC.src.data.transforms import PhotoTransforms, PathologyTransforms, TextTransforms
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            # 计算损失
            modal_availability = batch.get('modal_availability', 
                                          torch.ones(len(targets), 3, device=device))
        ...
        print(f"Epoch {epoch+1:3d}/{num_epochs}: Loss = {avg_loss:.6f}, Accuracy = {accuracy:.1f}%")
    ...
    print(f"\n判定标准:")
    print(f"  1. 最终损失 < 0.1: {'✅ 通过' if final_loss < 0.1 else '❌ 失败'} (实际: {final_loss:.6f})")
    print(f"  2. 预测正确: {'✅ 通过' if pred_label == true_label else '❌ 失败'}")
    print(f"  3. 置信度 > 0.9: {'✅ 通过' if confidence > 0.9 else '⚠️  警告'} (实际: {confidence:.4f})")
