...
def test_data_loading():
    ...
    try:
        ...
        # 验证文件存在性
        all_files_exist = (
            Path(patient_data['text_path']).exists() and
            all(Path(p).exists() for p in patient_data['photo_paths']) and
            all(Path(p).exists() for p in patient_data['pathology_paths'])
        )
...
def test_inference_pipeline(model, patient_data, model_config):
    ...
    try:
        ...
        # 堆叠照片并添加batch维度
        if photo_tensors:
            photos_input = torch.stack(photo_tensors).unsqueeze(0).to(device)  # [1, N, 3, 224, 224]
        ...
        pathology_transform = PathologyTransforms(is_training=False)  # 推理模式，无数据增强
        ...
        for i, path_path in enumerate(patient_data['pathology_paths'], 1):
            try:
                ...
                all_patches.extend(patches_from_slide[:10])  # 限制每个切片最多10个patches
                print(f"  ✅ 病理切片{i}加载成功，提取了{len(patches_from_slide)}个patches")
        ...
        # 堆叠patches并添加batch维度
        if all_patches:
            pathology_patches = torch.stack(all_patches).unsqueeze(0).to(device)  # [1, num_patches, 3, 224, 224]
        ...
        batch = {
            'text': {
                ...
                'lengths': torch.tensor([len(patient_data['text'])], dtype=torch.long).to(device)
            },
            'photos': {
                'images': photos_input,  # [1, N, 3, 224, 224]
                'counts': torch.tensor([len(photo_tensors)], dtype=torch.long).to(device),
        ...
        if 'auxiliary_outputs' in outputs:
            ...
            for modal, aux_logits in aux_outputs.items():
                ...
                print(f"    {modal}: 预测={aux_pred.item()}, 置信度={aux_probs.max().item():.4f}")
            ...
            if isinstance(attn_weights, dict):
                for key, weight in attn_weights.items():
                    if torch.is_tensor(weight):
                        print(f"    {key}: 形状={weight.shape}, 均值={weight.mean().item():.4f}")
        ...
        if 'feature_activations' in interp_info:
            ...
            for modal, activation in interp_info['feature_activations'].items():
                if torch.is_tensor(activation):
                    print(f"    {modal}: 形状={activation.shape}, 均值={activation.mean().item():.4f}")
...
def test_batch_inference(model, model_config):
    ...
    try:
        ...
        batch = {
            ...
            'pathology': {
                'patches': torch.randn(batch_size, 8, 3, 224, 224).to(device),  # 每个样本8个patches
        ...
        if 'logits' in outputs:
            ...
            for i in range(batch_size):
                print(f"    样本{i+1}: 类别={pred_classes[i].item()}, 置信度={confidences[i].item():.4f}")
...
def main():
    ...
    # 4. 可解释性测试
    test4_success, interp_info = test_interpretability(model, patient_data, model_config)
