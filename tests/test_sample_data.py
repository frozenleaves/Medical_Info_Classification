            ...
            for text_file in text_files:
                try:
                    ...
                    if content:
                        ...
                        print(f"  ✅ {text_file.name}: {file_size} bytes, {char_count} 字符")
        ...
        try:
            ...
            if photos_dir.exists():
                for patient_dir in photos_dir.iterdir():
                    if patient_dir.is_dir():
                        patient_photos = list(patient_dir.glob('*.png')) + list(patient_dir.glob('*.jpg'))
            ...
            if pathology_dir.exists():
                for patient_dir in pathology_dir.iterdir():
                    if patient_dir.is_dir():
                        patient_slides = list(patient_dir.glob('*.tif')) + list(patient_dir.glob('*.tiff'))
                        pathology_count += len(patient_slides)
                        print(f"  🔬 {patient_dir.name}: {len(patient_slides)} 张病理切片")
                        
                        for slide in patient_slides:
                            file_size = slide.stat().st_size
                            print(f"    - {slide.name}: {file_size} bytes")
            
            print(f"✅ 总计图像文件: 照片 {photo_count} 张, 病理切片 {pathology_count} 张")
    ...
    def test_data_consistency(self):
        ...
        try:
            ...
            for patient_id in labels.keys():
                ...
                if photo_dir.exists():
                    photos = list(photo_dir.glob('*.png')) + list(photo_dir.glob('*.jpg'))
                    patient_data['has_photos'] = len(photos) > 0
                
                # 检查病理文件
                pathology_dir = self.data_dir / 'pathology' / patient_id
                if pathology_dir.exists():
                    slides = list(pathology_dir.glob('*.tif')) + list(pathology_dir.glob('*.tiff'))
            ...
            for patient_id, data in consistency_results.items():
                ...
                print(f"  {patient_id}: 文本{text_status} 照片{photo_status} 病理{pathology_status} ({completeness})")
            
            total_patients = len(consistency_results)
            completeness_rate = (complete_patients / total_patients * 100) if total_patients > 0 else 0
        ...
        try:
            ...
            if (self.data_dir / 'photos').exists():
                ...
                for patient_dir in (self.data_dir / 'photos').iterdir():
                    if patient_dir.is_dir():
                        photos = list(patient_dir.glob('*.png')) + list(patient_dir.glob('*.jpg'))
            ...
            if (self.data_dir / 'pathology').exists():
                ...
                for patient_dir in (self.data_dir / 'pathology').iterdir():
                    if patient_dir.is_dir():
                        slides = list(patient_dir.glob('*.tif')) + list(patient_dir.glob('*.tiff'))
